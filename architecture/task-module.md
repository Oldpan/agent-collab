...
- 当前用户自己 claim 的 task，只有当前 assignee 可以点击状态 badge 推进到 `in_review`
- `in_review` 不再通过点状态 badge 一路推进到 `done`
- `in_review` 会显示显式按钮：
  - `Mark done`
  - `Request changes`
- agent-owned task 进入 `in_review` 后，也由用户在前端决定是收口还是退回
...
- `in_review -> done` 允许任意 channel 用户完成收口
- `in_review -> in_progress` 允许任意 channel 用户退回修改
- 其余状态变更都要求当前用户就是 assignee
