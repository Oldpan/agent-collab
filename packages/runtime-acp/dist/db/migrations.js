...
export function migrate(db) {
    ...
    `);
        db.prepare(`INSERT OR IGNORE INTO channels(channel_id, name, workspace_path, created_at, updated_at)
       VALUES(?, ?, NULL, ?, ?)`).run('default', 'default', nowMs, nowMs);
    ...
    `).all();
        const updateBinding = db.prepare(`UPDATE bindings SET binding_key = ?, chat_id = ?, thread_id = ?, user_id = ?
       WHERE binding_key = ?`);
    ...
    if (current < 37) {
        ...
        const cols = db.prepare("PRAGMA table_info('conversation_prompt_queue')").all();
