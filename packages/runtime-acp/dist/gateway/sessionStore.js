...
export function createSession(db, params) {
    ...
    VALUES(?, ?, ?, NULL, ?, ?, ?, ?)
    `).run(params.sessionKey, params.agentCommand, JSON.stringify(params.agentArgs), params.loadSupported ? 1 : 0, params.cwd, now, now);
}
...
export function getSession(db, sessionKey) {
    const row = db
        .prepare('SELECT session_key as sessionKey, agent_command as agentCommand, agent_args_json as agentArgsJson, acp_session_id as acpSessionId, cwd, load_supported as loadSupported FROM sessions WHERE session_key = ?')
        .get(sessionKey);
    return row ?? null;
}
