...
describe('REST API', () => {
  ...
  it('POST /api/conversations/:id/open-thread 在缺失 snapshot 时不应把普通触发消息当成 DM thread root', async () => {
    const now = Date.now();
    const agent = manager.createAgent({
      name: 'DmThreadResolverNoSnapshot',
      agentType: 'claude_acp',
      nodeId: 'node-1',
      workspacePath: '/tmp/dm-thread-resolver-no-snapshot',
    });
    const conv = manager.openAgentThread(agent.agentId);
    if (!conv) throw new Error('missing conversation');
    ...
    db.prepare(
      `INSERT INTO channel_messages(message_id, channel_id, sender_id, sender_name, sender_type, target, content, seq, created_at)
       VALUES('client-trigger-1111-0000-0000-000000000000', ?, 'user', 'oldpan', 'user', 'dm:@oldpan', '请把这个变成一个任务', 1, ?)`,
    ).run(dmChannelId, now);

    const result = await fetchJson(`/api/conversations/${conv.id}/open-thread`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ messageId: 'client-trigger-1111-0000-0000-000000000000' }),
    });

    expect(result.status).toBe(404);
    expect(result.body.error).toBe('Thread root not found in this DM');
  });
