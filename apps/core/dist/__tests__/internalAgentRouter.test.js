...
describe('internalAgentRouter', () => {
    it('send_message 应把消息绑定到当前会话的 active run', async () => {
        ...
        const row = db.prepare('SELECT run_id as runId, channel_id as channelId, message_kind as messageKind, message_source as messageSource FROM channel_messages WHERE sender_id = ? ORDER BY created_at DESC LIMIT 1').get(agent.agentId);
    ...
    it('thread conversation 中的 agent 仍可读取所属主频道历史', async () => {
       ...
       ('thread-msg-1', 'default', 'user', 'User', 'user', '#default:mainread1', 'thread-only context', 2, ?)`).run(Date.now(), Date.now() + 1);
        const res = await fetch(`${baseUrl}/api/internal/agent/${agent.agentId}/history?channel=${encodeURIComponent('#default')}`);
        expect(res.status).toBe(200);
        const body = await res.json();
