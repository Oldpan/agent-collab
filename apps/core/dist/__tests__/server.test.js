        ...
        try {
            ...
            for (const agentId of body.agentIds ?? []) {
                if (manager.getAgent(agentId))
                    manager.joinChannel(agentId, channel.channelId);
            }
            reply.code(201);
            return channel;
        }
           ...
           LIMIT 1`).get(req.params.id, threadRootId);
            if (rootMsg?.sender_type === 'agent') {
                ...
                if (conv) {
                    ...
                    void manager.submitPrompt(conv.id, buildChannelActivationPrompt({
...
describe('REST API', () => {
    ...
    it('POST /api/channels/:id/messages 在 subscribed_agents 模式下，无 root participants 时应唤醒订阅者', async () => {
        ...
        const alicePrompt = db.prepare(`SELECT r.prompt_text as promptText
       FROM runs r
       JOIN conversations c ON c.session_key = r.session_key
       WHERE c.id = ?
       ORDER BY r.started_at DESC
       LIMIT 1`).get(aliceConv?.id);
        const bobPrompt = db.prepare(`SELECT r.prompt_text as promptText
       FROM runs r
       JOIN conversations c ON c.session_key = r.session_key
       WHERE c.id = ?
       ORDER BY r.started_at DESC
       LIMIT 1`).get(bobConv?.id);
    ...
    it('POST /api/channels/:id/messages 在 subscribed_agents 模式下，有 root participants 时应优先唤醒参与者', async () => {
        ...
        const ownerPrompt = db.prepare(`SELECT r.prompt_text as promptText
       FROM runs r
       JOIN conversations c ON c.session_key = r.session_key
       WHERE c.id = ?
       ORDER BY r.started_at DESC
       LIMIT 1`).get(ownerConv?.id);
        expect(ownerPrompt?.promptText).toContain('There is new channel activity');
        const watcherRunCount = db.prepare(`SELECT COUNT(*) as count
       FROM runs r
       JOIN conversations c ON c.session_key = r.session_key
       WHERE c.agent_id = ? AND c.channel_id = ?`).get(watcher.agentId, channel.channelId);
    ...
    it('POST /api/channels/:id/messages 在主频道 @agent 时应直接把触发消息写进激活 prompt', async () => {
        ...
        if (!conv)
            throw new Error('missing mention conversation');
        const sessionRow = db
            .prepare('SELECT session_key as sessionKey FROM conversations WHERE id = ?')
            .get(conv.id);
        const runRow = db.prepare(`SELECT prompt_text as promptText
       FROM runs
       WHERE session_key = ?
       ORDER BY started_at DESC
       LIMIT 1`).get(sessionRow.sessionKey);
    ...
    it('POST /api/channels/:id/messages 在 thread reply 时应直接把 reply 内容写进激活 prompt', async () => {
        ...
        db.prepare(`INSERT INTO channel_messages(message_id, channel_id, sender_id, sender_name, sender_type, target, content, seq, created_at, run_id, thread_root_id)
       VALUES(?, 'default', ?, ?, 'agent', '#default', ?, 1, ?, NULL, NULL)`).run(rootMessageId, agent.agentId, agent.name, 'root message', Date.now());
        const response = await fetchJson('/api/channels/default/messages', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        expect(conv).not.toBeNull();
        if (!conv)
            throw new Error('missing thread reply conversation');
        const sessionRow = db
            .prepare('SELECT session_key as sessionKey FROM conversations WHERE id = ?')
            .get(conv.id);
        const runRow = db.prepare(`SELECT prompt_text as promptText
