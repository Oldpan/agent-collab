...
describe('nodeWsHandler', () => {
    ...
    it('已发送 final 后若继续输出实质性 delta 且没有更新的 final，应触发 trailing-final repair', async () => {
       ...
       VALUES(?, ?, 'node/event', ?, ?)`).run('run-trailing-final-1', 1, JSON.stringify({
        ...
        db.prepare(`INSERT INTO channel_messages(message_id, channel_id, sender_id, sender_name, sender_type, target, content, seq, created_at, run_id, message_kind)
       VALUES(?, ?, ?, ?, 'agent', ?, ?, ?, ?, ?, ?)`).run('msg-trailing-final-repair-1', `dm:${agent.agentId}`, agent.agentId, agent.name, 'dm:@oldpan', '1. base - 基础环境\n2. develop - 开发环境\n3. vllm - 推理环境', 2, Date.now(), repairDispatch.runId, 'final');
        socket.emit('message', JSON.stringify({
            type: 'run.end',
            runId: repairDispatch.runId,
            conversationId: conv.id,
