...
describe('nodeWsHandler', () => {
    ...
    it('已发送 final 后若继续输出实质性 delta，应追加 delta_fallback 消息', () => {
       ...
       VALUES(?, ?, ?, ?, 'agent', ?, ?, ?, ?, ?, ?)`).run('msg-trailing-final-1', `dm:${agent.agentId}`, agent.agentId, agent.name, 'dm:@oldpan', '你的 conda 环境列表：', 1, 1000, 'run-trailing-final-1', 'final');
        db.prepare(`INSERT INTO events(run_id, seq, method, payload_json, created_at)
       VALUES(?, ?, 'node/event', ?, ?)`).run('run-trailing-final-1', 1, JSON.stringify({
            type: 'content.delta',
            text: '1. base - 基础环境\n2. develop - 开发环境\n3. vllm - 推理环境',
        }), 2000);
        socket.emit('message', JSON.stringify({
       ...
       ORDER BY created_at DESC, seq DESC
       LIMIT 1`).get('run-trailing-final-1');
    ...
    it('已发送 final 后收到 cancel stopReason 时应按成功收口，不触发 repair', () => {
        ...
        db.prepare(`INSERT INTO channel_messages(message_id, channel_id, sender_id, sender_name, sender_type, target, content, seq, created_at, run_id, message_kind)
       VALUES(?, ?, ?, ?, 'agent', ?, ?, ?, ?, ?, ?)`).run('msg-cancel-final-1', `dm:${agent.agentId}`, agent.agentId, agent.name, 'dm:@oldpan', 'done', 1, Date.now(), 'run-cancel-final-1', 'final');
        socket.emit('message', JSON.stringify({
            type: 'run.end',
            runId: 'run-cancel-final-1',
            conversationId: conv.id,
    ...
    it('未发送 final 就 cancel 时应按失败收口，且不触发 repair', () => {
        ...
        db.prepare(`INSERT INTO channel_messages(message_id, channel_id, sender_id, sender_name, sender_type, target, content, seq, created_at, run_id, message_kind)
       VALUES(?, ?, ?, ?, 'agent', ?, ?, ?, ?, ?, ?)`).run('msg-cancel-progress-1', `dm:${agent.agentId}`, agent.agentId, agent.name, 'dm:@oldpan', 'working', 1, Date.now(), 'run-cancel-no-final-1', 'progress');
        socket.emit('message', JSON.stringify({
            type: 'run.end',
            runId: 'run-cancel-no-final-1',
            conversationId: conv.id,
    ...
    it('仅发送 progress 消息且后续仍有大量输出时应追加 delta_fallback 消息并正常收口', () => {
       ...
       VALUES(?, ?, 'node/event', ?, ?)`).run('run-3', 1, JSON.stringify({
        ...
        expect(originalRun.error).toBeNull();
        expect(originalRun.stopReason).toBe('end_turn');
        const rows = db.prepare(`SELECT content, message_kind as messageKind, message_source as messageSource
       FROM channel_messages
       WHERE run_id = ?
       ORDER BY created_at ASC, seq ASC`).all('run-3');
    ...
    it('旧式未标注 kind 的单条回复若与后续输出只是重复，不应误判缺少 final reply', () => {
       ...
       VALUES(?, ?, 'node/event', ?, ?)`).run('run-legacy-1', 1, JSON.stringify({
        ...
        const runRow = db.prepare('SELECT error, stop_reason as stopReason FROM runs WHERE run_id = ?')
            .get('run-legacy-1');
        const rows = db.prepare(`SELECT COUNT(*) as count FROM channel_messages WHERE run_id = ?`).get('run-legacy-1');
        expect(runRow.error).toBeNull();
        expect(runRow.stopReason).toBe('end_turn');
    ...
    it('channel branch run 未通过 send_message 回复时应追加频道 delta_fallback 消息', () => {
       ...
       WHERE run_id = ? AND message_source = 'delta_fallback'
       ORDER BY created_at DESC, seq DESC
       LIMIT 1`).get('run-channel-repair-1');
