...
describe('ExecutionDispatcher', () => {
    ...
    it('私聊用户消息应直接作为激活 prompt 下发，并推进 DM root checkpoint', async () => {
        ...
        const msgRow = db.prepare('SELECT content, seq FROM channel_messages WHERE channel_id = ? ORDER BY seq DESC LIMIT 1').get(dmChannelId);
        expect(msgRow?.content).toBe('你好，帮我总结一下刚才的结论');
        const checkpointRow = db.prepare('SELECT last_seq as lastSeq FROM agent_message_checkpoints WHERE agent_id = ? AND channel_id = ? AND thread_root_id = ?').get(agent.agentId, dmChannelId, '');
    ...
    it('同一 agent 的第二个 thread 提交时应进入 queued', async () => {
        ...
        const queueEntry = db.prepare(`SELECT conversation_id as conversationId, prompt_text as promptText,
    ...
    it('排队的 prompt 在重新派发时应保留 activationContextText', async () => {
        ...
        expect(queued.queued).toBe(true);
        const queueEntry = db.prepare(`SELECT activation_context_text as activationContextText
       FROM conversation_prompt_queue
       WHERE conversation_id = ?`).get(branch.id);
        expect(queueEntry?.activationContextText).toBe('[Thread root message]\nhello root');
        const firstDispatch = sent[0]?.msg;
        if (!firstDispatch || firstDispatch.type !== 'run.dispatch')
            throw new Error('missing first dispatch');
        ...
        if (!secondDispatch || secondDispatch.type !== 'run.dispatch')
            throw new Error('missing second dispatch');
