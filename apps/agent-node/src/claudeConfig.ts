...
export function getIsolatedClaudeConfigDir(workspaceRoot: string): string {
  return path.join(path.resolve(workspaceRoot), CLAUDE_RUNTIME_DIRNAME);
}

export function getIsolatedClaudeStatePath(workspaceRoot: string): string {
  return `${getIsolatedClaudeConfigDir(workspaceRoot)}.json`;
}

export function ensureIsolatedClaudeConfig(workspaceRoot: string): string {
